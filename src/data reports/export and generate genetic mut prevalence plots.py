...
def plot_prevalence_by_subtype(
    df, per_row, top_genes_list, out_png, subtype_col="SUBTYPE"
):
...
def main():
    ...
    # save counts
    pd.DataFrame(counts.most_common()).rename(columns={0: "gene", 1: "count"}).to_csv(
        os.path.join(args.outdir, "gene_counts.csv"), index=False
    )
